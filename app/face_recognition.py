...
def faceRecognitionPipeline(filename, path=True):
    ...
    for x,y,w,h in faces:
        ...
        if roi.shape[1] > 100:
            roi_size = cv2.resize(roi, (100, 100), cv2.INTER_AREA)
        else:
            roi_size = cv2.resize(roi, (100, 100), cv2.INTER_CUBIC)
        # step-06: Flattening (1X10000)
        roi_reshape = roi_size.reshape(1, 10000)
        # step-07: subtract with mean
        roi_mean = roi_reshape - mean_face_arr # subtrace face with mean face
        # step-08: get eigen image (apply roi_main to pca)
